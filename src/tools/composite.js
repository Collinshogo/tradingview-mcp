...
export function registerCompositeTools(server) {
  ...
  server.tool('strategy_deep_run', 'The only saved-My-Scripts strategy attachment route: removes prior strategies and verifies two consecutive zero-strategy scans, opens the named saved script in the editor and verifies its exact Pine script ID, clicks Add to chart, then accepts only a newly attached entity with that exact ID. It then runs a Deep Backtest over a date range. Optionally overrides inputs, arms the range, clicks "Update Report" when needed, and polls until a fresh deep report or terminal TradingView error is served. The default through_latest policy follows the required START-DATE-ONLY method and never types a system-clock end date; exact must be requested explicitly for a frozen historical/parity window. The result carries served_to.', {
